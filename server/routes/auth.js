...
router.post('/login', async (req, res) => {
  ...
  res.json({ token, agent: { id: agent.id, email: agent.email, name: agent.name, phone: agent.phone, company: agent.company, is_admin: agent.is_admin || 0, agency_id: agent.agency_id || null }, agency });
});
...
router.get('/me', authenticate, async (req, res) => {
  const db = getDb();
  const agent = await db.prepare('SELECT id, email, name, phone, company, is_admin, gmail_email, agency_id, created_at FROM agents WHERE id = ?').get(req.agent.id);
...
router.put('/me', authenticate, async (req, res) => {
  ...
  if (newPassword) {
    ...
    await db.prepare('UPDATE agents SET name = ?, phone = ?, company = ?, password = ? WHERE id = ?').run(name || agent.name, phone || agent.phone, company || agent.company, hash, req.agent.id);
  } else {
    await db.prepare('UPDATE agents SET name = ?, phone = ?, company = ? WHERE id = ?').run(name || agent.name, phone || agent.phone, company || agent.company, req.agent.id);
  }
...
router.get('/admin/agents', authenticate, async (req, res) => {
  ...
  const agents = await db.prepare(`
    SELECT a.id, a.email, a.name, a.phone, a.company, a.is_admin, a.gmail_email, a.agency_id, a.created_at,
